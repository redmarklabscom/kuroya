...
#[test]
fn normal_mode_visual_character_shift_comma_outdents_selected_line_span_and_repeats() {
    ...
    let outdent = handle_vim_editor_key_event_with_state_and_indent(
        &mut buffer,
        Key::Comma,
        Modifiers::SHIFT,
        &mut mode,
    ...
    assert!(outdent.handled);
    assert!(outdent.changed);
    assert_eq!(outdent.suppress_text, Some('<'));
    assert_eq!(
        buffer.text(),
        "  one\n  two\n  three\n    four\n    five\n    six\n"
    );
    ...
    buffer.set_single_cursor(buffer.line_column_to_char(3, 2));
    let repeat = handle_vim_editor_key_event_with_state_and_indent(
        &mut buffer,
        Key::Period,
        Modifiers::NONE,
    ...
    assert_eq!(buffer.cursor(), buffer.line_column_to_char(3, 0));
    assert!(pending.is_none());
    assert!(vim_events_include_mutation(
        &[
            key_event(Key::V, Modifiers::NONE),
            key_event(Key::J, Modifiers::NONE),
            key_event(Key::J, Modifiers::NONE),
            key_event(Key::Comma, Modifiers::SHIFT),
        ],
        EditorVimMode::Normal,
        None,
    ));
