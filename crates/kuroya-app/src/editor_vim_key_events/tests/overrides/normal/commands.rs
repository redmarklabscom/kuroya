...
#[test]
fn normal_mode_vim_settings_override_custom_key_to_command() {
    ...
    assert_eq!(result.suppress_text, Some('K'));
...
#[test]
fn normal_mode_vim_settings_override_ctrl_key_to_command() {
    let mut buffer = TextBuffer::from_text(1, None, "alpha".to_owned());
    buffer.set_single_cursor(1);
    let mut mode = EditorVimMode::Normal;
    let mut pending = None;
