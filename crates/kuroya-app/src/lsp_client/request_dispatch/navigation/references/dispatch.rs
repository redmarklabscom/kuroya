...
use super::pending::register_references_request;
...
pub(super) async fn dispatch_references(
    id: BufferId,
    path: PathBuf,
    version: u64,
    line: usize,
    ...
    let message =
        LspWireMessage::references(request_id, &path, line, character, include_declaration)
            .to_json();
    register_references_request(
        request_id,
        id,
        path,
        version,
